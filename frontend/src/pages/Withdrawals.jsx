...
const EMPTY_PM = { type: "upi", upi_id: "", account_holder: "", account_number: "", ifsc: "", bank_name: "" };

const PaymentMethodRow = ({ pm, onDelete }) => (
  <div className="px-6 py-4 flex items-center gap-4">
    <div className="w-10 h-10 rounded-md bg-secondary border border-border flex items-center justify-center">
      {pm.type === "upi" ? <CreditCard size={18}/> : <Bank size={18}/>}
    </div>
    <div className="flex-1 min-w-0">
      <div className="text-sm font-semibold uppercase tracking-wider">{pm.type}</div>
      <div className="mono text-xs text-muted-foreground truncate">
        {pm.type === "upi" ? pm.upi_id : `${pm.bank_name || "Bank"} · ${pm.account_number?.slice(-4).padStart(pm.account_number?.length, "•")} · ${pm.ifsc}`}
      </div>
    </div>
    <button onClick={() => onDelete(pm.id)} className="p-2 text-muted-foreground hover:text-destructive rounded-md hover:bg-destructive/10">
      <Trash size={16}/>
    </button>
  </div>
);

const AddPaymentMethodForm = ({ pmForm, setPmForm, onSubmit }) => (
  <form onSubmit={onSubmit} className="p-6 border-b border-border space-y-3 bg-surface">
...
const NewWithdrawalForm = ({ wForm, setWForm, pms, balance, onSubmit }) => (
...
export default function Withdrawals() {
  ...
  const delPM = async (id) => {
    ...
    try { await api.delete(`/payment-methods/${id}`); load(); } catch (ex) { setErr(fmtErr(ex)); }
  };
    ...
    <div className="p-6 lg:p-10 max-w-7xl mx-auto fade-up">
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-8">
        <div className="card-flat">
          ...
          {showAddPM && <AddPaymentMethodForm pmForm={pmForm} setPmForm={setPmForm} onSubmit={addPM}/>}
          <div className="divide-y divide-border">
            {pms.length === 0 ? (
              <div className="p-8 text-center text-sm text-muted-foreground">No methods yet. Add one to withdraw.</div>
            ) : pms.map((p) => <PaymentMethodRow key={p.id} pm={p} onDelete={delPM}/>)}
          </div>
        </div>

        {showWithdraw && (
          <NewWithdrawalForm wForm={wForm} setWForm={setWForm} pms={pms} balance={balance} onSubmit={submitWithdraw}/>
        )}
      </div>
