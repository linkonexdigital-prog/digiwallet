    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="lg:hidden sticky top-0 z-30 flex items-center justify-between border-b border-border bg-background px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button data-testid="mobile-menu-toggle" onClick={() => setMobileOpen(true)} className="p-2 rounded-md hover:bg-secondary">
            <List size={20}/>
          </button>
        </div>
      </header>
      ...
      <div className="flex">
        ...
        <aside className="hidden lg:flex w-64 shrink-0 h-screen sticky top-0 border-r border-border flex-col bg-card">
          ...
          <nav className="flex-1 p-3 space-y-1">
            <div className="overline text-muted-foreground px-3 py-2">Navigation</div>
            {navItems.map((it) => (
              <NavLink
          ...
          <div className="p-3 border-t border-border space-y-2">
            ...
            {adminBackup && (
              <button onClick={restoreAdmin} data-testid="restore-admin-btn" className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm bg-warning/10 text-warning hover:bg-warning/20 transition">
            ...
            <div className="flex items-center gap-3 px-3 py-2">
              ...
              <button data-testid="logout-button" onClick={logout} className="p-2 rounded-md hover:bg-secondary text-muted-foreground hover:text-destructive">
                <SignOut size={16}/>
              </button>
            </div>
          </div>
        ...
        {mobileOpen && (
          <div className="lg:hidden fixed inset-0 z-40 bg-background/80 backdrop-blur-sm" onClick={() => setMobileOpen(false)}>
            <div className="absolute top-0 left-0 w-72 h-full bg-card border-r border-border p-4 fade-up" onClick={(e) => e.stopPropagation()}>
              ...
              <button onClick={logout} className="mt-4 w-full flex items-center gap-2 px-3 py-2.5 rounded-md text-sm text-destructive hover:bg-destructive/10">
